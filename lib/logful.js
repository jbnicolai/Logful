...
function Logful (module) {
  ...
  { get: function getIdentity () {
      var identity = Logful.application
...
Logful.use = function use (handler, opts) {
  ...
  // Normalise opts
  opts = opts || {}
  opts.formats = merge(Logful.formats, opts.formats) // If custom styles were not defined, use defaults
...
Logful.prototype.log = function log (level, message) {
   ...
   * @property  {String}   identity            Combination of application's and module's name, in the form App\Module
